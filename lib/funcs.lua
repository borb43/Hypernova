...
HPR.get_moon_scoring = function (moon_card)
    local mult = G.GAME.hpr_moons_mult or 1
    local ret = {}
    ret.chips = (moon_card.ability.moon_bonus or 0)*mult + (moon_card.ability.moon_h_chips or 0)*mult
    ret.chips = ret.chips ~= 0 and ret.chips or nil
    ret.mult = (moon_card.ability.moon_mult or 0)*mult + (moon_card.ability.moon_h_mult or 0)*mult
    ret.mult = ret.mult ~= 0 and ret.mult or nil
    ret.xchips = (moon_card.ability.moon_x_chips or 0)*mult + (moon_card.ability.moon_h_x_chips or 0)*mult + 1
    ret.xchips = ret.xchips ~= 1 and ret.xchips or nil
    ret.xmult = (moon_card.ability.moon_x_mult or 0)*mult + (moon_card.ability.moon_h_x_mult or 0)*mult + 1
    ret.xmult = ret.xmult ~= 1 and ret.xmult or nil
    ret.dollars = (moon_card.ability.moon_p_dollars or 0)*mult + (moon_card.ability.moon_h_dollars or 0)*mult
    ret.dollars = ret.dollars ~= 0 and ret.dollars or nil
    if next(SMODS.find_mod('entr')) then
        ret.plus_asc = (moon_card.ability.moon_entr_plus_asc or 0)*mult + (moon_card.ability.moon_entr_h_plus_asc or 0)*mult
        ret.plus_asc = ret.plus_asc ~= 0 and ret.plus_asc or nil
        ret.asc = (moon_card.ability.moon_entr_asc or 0)*mult + (moon_card.ability.moon_entr_h_asc or 0)*mult
        ret.asc = ret.asc ~= 1 and ret.asc or nil
        ret.exp_asc = (moon_card.ability.moon_entr_exp_asc or 0)*mult + (moon_card.ability.moon_entr_h_exp_asc or 0)*mult
        ret.exp_asc = ret.exp_asc ~= 1 and ret.exp_asc or nil
    end
    if next(SMODS.find_mod("aikoyorisshenanigans")) then
        ret.akyrs_score = (moon_card.ability.akyrs_moon_score or 0)*mult + (moon_card.ability.akyrs_moon_h_score or 0)*mult
        ret.akyrs_score = ret.akyrs_score ~= 0 and ret.akyrs_score or nil
    end
    return ret
end